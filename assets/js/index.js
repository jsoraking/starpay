$(document).ready(function () {
  ...
  $('.fullpage-wrap').fullpage({
    ...
    afterLoad: function (origin, direction) {
      ...
      if (origin == 'slide1') {
        ...
        $('.block-chain .animation.from-left-out').removeClass('from-left-out').addClass('from-left-in');
        $('.block-chain .animation.from-right-out').removeClass('from-right-out').addClass('from-right-in');
        $('.block-chain .animation.from-bottom-out').removeClass('from-bottom-out').addClass('from-bottom-in');
      }
      // 첫번째 섹션이 아닐 때,
        ...
        if (origin == 'slide2') {
          ...
          $('.star-wallet .animation.from-left-out').removeClass('from-left-out').addClass('from-left-in');
        }
        else if (origin == 'slide3') {
          $('.face-recog .animation.from-bottom-out').removeClass('from-bottom-out').addClass('from-bottom-in');
          $('.face-recog .animation.from-right-out').removeClass('from-right-out').addClass('from-right-in');
        }
        else if (origin == 'slide4') {
          $('.easy-everywhere .animation.scale-down-out').removeClass('scale-down-out').addClass('scale-down-in');
          $('.easy-everywhere .animation.from-right-out').removeClass('from-right-out').addClass('from-right-in');
        }
        else if (origin == 'slide5') {
          $('.safety-fintech .animation.from-bottom-out').removeClass('from-bottom-out').addClass('from-bottom-in');
          $('.safety-fintech .animation.scale-down-out').removeClass('scale-down-out').addClass('scale-down-in');
        }
        else if (origin == 'slide6') {
          $('.starpay-use .animation.from-bottom-out').removeClass('from-bottom-out').addClass('from-bottom-in');
          $('.starpay-use .animation.from-left-out').removeClass('from-left-out').addClass('from-left-in');
        }
      }
    },
  ...
  var bgLengthCheck = $('.mo-menu__bg').length;
  ...
  $('.js-hd-menu-btn').click(function () {
    if (hdMenu == false) {
      ...
      if (bgLengthCheck == '0') {
        $('.js-header').append('<div class="mo-menu__bg"></div>');
      }
    }
  });
  //메뉴>하위메뉴 펼치고 접기
